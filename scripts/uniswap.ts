...
async function main() {
  ...
  const paths3 = [DAI, "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"]
  const approval = await DaiContract.connect(impersonatedSigner).approve(ROUTER, 100000000)
  await Uniswap.connect(impersonatedSigner).swapTokensForExactETH(100, 100000000, paths3, DAIHolder,time);
  
  const ETHbalanceAfterTokenSwap = await ethers.provider.getBalance(DAIHolder);
  console.log(`ETHBalance after eth swap ${ETHbalanceAfterTokenSwap}`);

  const holderBalanceAfterETHsWap = await DaiContract.balanceOf(DAIHolder);
  console.log(`Dai balance After ETH swap ${holderBalanceAfterETHsWap}`);
  console.log()


 const paths4 = [DAI, UNI]
 const approval1 = await DaiContract.connect(impersonatedSigner).approve(ROUTER, 100000000)
  await Uniswap.connect(impersonatedSigner).swapTokensForExactTokens(100, 100000000, paths4, DAIHolder, time);

  const holderBalanceOfDAIAfterUNI = await DaiContract.balanceOf(DAIHolder);
  console.log(`Dai balance After UNI swap ${holderBalanceOfDAIAfterUNI}`);

  const uniBalanceAfterDAI = await UniContract.balanceOf(DAIHolder);
  console.log(`uniBalanceAfter DAI swap ${uniBalanceAfterDAI}`);
}
